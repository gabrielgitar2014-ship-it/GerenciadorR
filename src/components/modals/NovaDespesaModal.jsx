...
const METODOS_DE_PAGAMENTO = ["Bradesco", "Itaú", "Nubank", "Pix", "Outro"];
...
const getInitialState = (despesaParaEditar = null) => {
  if (despesaParaEditar) {
    // Para edição, busca o mes_inicio_cobranca salvo ou calcula a partir da data da compra
...
export default function NovaDespesaModal({ onClose, onSave, despesaParaEditar, cardConfigs = [] }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const dadosParaSalvar = {
        ...
        // A linha crucial que salva o dado na nova coluna
        mes_inicio_cobranca: formData.mes_inicio_cobranca,
    };

    try {
      if (isEditMode) {
        const { error: deleteParcelasError } = await supabase.from('parcelas').delete().eq('despesa_id', despesaParaEditar.id);
        if (deleteParcelasError) throw deleteParcelasError;

        const { error: deleteError } = await supabase.from('despesas').delete().eq('id', despesaParaEditar.id);
