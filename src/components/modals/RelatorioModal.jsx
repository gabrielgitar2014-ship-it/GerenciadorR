...
// Importando nossa lógica de processamento do arquivo de utilitários
import { processarDadosRelatorio } from '../../utils/reportUtils'; 

// Componente para os botões de preset de data
...
export default function RelatorioModal({ onClose }) {
    ...
    const [apenasParceladas, setApenasParceladas] = useState(false);
    
    const [reportData, setReportData] = useState(null);
    const [instance, updateInstance] = usePDF();

    // Deriva a lista de categorias dinamicamente
    ...
    // Função de gerar relatório agora utiliza a lógica externa
    const handleGenerateReport = useCallback(() => {
        setReportData(null); 
        updateInstance(null);
        
        const filtros = { startDate, endDate, metodoPagamento, categoria, apenasParceladas };
        const calculatedData = processarDadosRelatorio(allParcelas, filtros);
        
        if (calculatedData) {
            setReportData(calculatedData);
        }
    }, [startDate, endDate, metodoPagamento, categoria, apenasParceladas, allParcelas, updateInstance]);
    ...
    const handleDownload = () => {
        ...
        link.download = `Relatorio_Despesas.pdf`;
        link.click();
    };
    
    const isReady = reportData !== null && !instance.loading;
    const hasResults = isReady && reportData.despesas.length > 0;
    const hasNoResults = isReady && reportData.despesas.length === 0;

    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 animate-fade-in">
            <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-lg">
                <h2 className="text-2xl font-bold mb-4 text-gray-800">Gerar Relatório de Despesas</h2>
                ...
                <div className="space-y-4">
                    ...
                    {/* Filtro de despesas parceladas */}
                    <div className="flex items-center pt-2">
                        <input id="apenasParceladas" type="checkbox" checked={apenasParceladas} onChange={(e) => setApenasParceladas(e.target.checked)} className="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" />
                        <label htmlFor="apenasParceladas" className="ml-3 block text-sm text-gray-900">Mostrar apenas despesas parceladas</label>
                    </div>
                </div>

                {/* Área de feedback visual para o usuário */}
                <div className="mt-6 min-h-[60px] flex flex-col justify-center items-center">
                    {hasNoResults && ( <div className="p-3 bg-yellow-100 border border-yellow-400 text-yellow-800 rounded text-center animate-fade-in">Nenhuma despesa encontrada para os filtros selecionados.</div> )}
